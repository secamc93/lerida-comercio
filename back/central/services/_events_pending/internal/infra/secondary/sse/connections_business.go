...
func (m *EventManager) AddConnection(businessID uint, filter *entities.SSEConnectionFilter, conn http.ResponseWriter) string {
	m.mutex.Lock()
	defer m.mutex.Unlock()

	connectionID := fmt.Sprintf("conn_%d_%d", businessID, atomic.AddUint64(&m.connectionCounter, 1))

	connection := &entities.SSEConnection{
